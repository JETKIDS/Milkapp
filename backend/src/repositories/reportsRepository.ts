...
export interface DeliveryListFilter {
  ...
  endDate?: string;   // ISO
  courseId?: number;  // 後方互換性のため残す
  courseIds?: number[]; // 新しい複数選択フィールド
  manufacturerIds?: number[]; // メーカー選択
  outputType?: 'combined' | 'separate'; // 出力形式
}
...
export const reportsRepository = {
        ...
        const contractDeliveries = customer.contracts
          .filter(contract => {
            ...
            if (contract.cancelDate) {
              ...
              if (targetDate > cancelDate) return false;
            }
  ...
  async getDeliveryScheduleForMultipleCourses(courseIds: number[], startDate: string, endDate: string) {
    ...
    while (currentDate <= end) {
      ...
      for (const courseId of courseIds) {
        ...
        // コース情報を追加
        const courseInfo = await prisma.deliveryCourse.findUnique({
          where: { id: courseId }
        });
  ...
  async getCourseInfo(courseId: number) {
    return prisma.deliveryCourse.findUnique({
      where: { id: courseId }
    });
  },
  ...
  async getProductList(filter: ProductListFilter) {
    ...
    const productMap = new Map<number, {
      ...
      courseBreakdown?: Map<number, { courseName: string; quantity: number }>;
    }>();
    ...
    while (currentDate <= endDate) {
      ...
      contracts.forEach((contract: any) => {
        ...
        if (contract.cancelDate) {
          ...
          targetDate.setHours(0, 0, 0, 0);
          if (targetDate > cancelDate) return;
        }
